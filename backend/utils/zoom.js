...
async function handleEngagementEnded(engagementId) {
  ...
  if (existing?.localPath && fs.existsSync(existing.localPath)) {
    console.log(`⚠️ [${engagementId}] Already processed, skipping.`);
    return existing;
  }
  ...
  if (recording.transcript_url) {
    try {
      ...
      console.warn(`[${engagementId}] ⚠️ Could not fetch/parse transcript:`, err.message || err);
      // continue without transcript
    }
  } else {
    console.log(`[${engagementId}] ℹ️ No transcript_url present for recording`);
  }
...
module.exports = {
  ...
  getRecordingUrls,
  streamDownload,
  // Exporting getRecording for testing/other uses may be convenient
  getRecording,
};
